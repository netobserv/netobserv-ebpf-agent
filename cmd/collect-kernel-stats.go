...
type KernelStats struct {
	...
	CollectionDuration   time.Duration                 `json:"collection_duration_sec"`
...
type ProgramKernelStats struct {
	...
	Tag                  string         `json:"tag,omitempty"` // BPF bytecode hash (available from 4.13)
	...
	Memlock              uint64         `json:"memlock_bytes,omitempty"`         // Memory locked by program (available from 4.10)
	JitedSize            uint32         `json:"jited_size_bytes,omitempty"`      // JIT-compiled code size (available from 4.13)
	VerifiedInstructions uint32         `json:"verified_instructions,omitempty"` // Number of verified instructions (available from 5.16)
	NumMaps              int            `json:"num_maps,omitempty"`              // Number of associated maps (available from 4.15)
}

func collectKernelStats(duration time.Duration, interval time.Duration) (*KernelStats, error) {
	...
	logrus.Infof("Collecting kernel-space eBPF statistics for %v (sampling every %v)", duration, interval)
	...
	for progID, initialStat := range initialStats {
		...
		// Extract additional metadata (these don't change during runtime)
		memlock, _ := info.Memlock()
		jitedSize, _ := info.JitedSize()
		verifiedInsns, _ := info.VerifiedInstructions()
		mapIDs, _ := info.MapIDs()
		...
		finalStats[info.Name] = ProgramKernelStats{
			...
			RecursionMisses:      stats.RecursionMisses - initialStat.RecursionMisses,
			Memlock:              memlock,
			JitedSize:            jitedSize,
			VerifiedInstructions: verifiedInsns,
			NumMaps:              len(mapIDs),
...
func main() {
	var (
		...
		interval = flag.Duration("interval", 1*time.Second, "Sampling interval (not used, kept for compatibility)")
	...
	stats, err := collectKernelStats(*duration, *interval)
	if err != nil {
