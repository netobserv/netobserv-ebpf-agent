...
func StartGRPCProto(cfg *config.Agent, m *metrics.Metrics) (*GRPCProto, error) {
	exporter := GRPCProto{
		...
		maxFlowsPerMessage: cfg.Flows.GRPCMessageMaxFlows,
...
func randomizeTimer(cfg *config.Agent) time.Duration {
	if cfg.Flows.GRPCReconnectTimer <= 0 {
		return 0
	}
	timer := cfg.Flows.GRPCReconnectTimer
	if cfg.Flows.GRPCReconnectTimerRandomization <= 0 || cfg.Flows.GRPCReconnectTimerRandomization >= timer {
		return timer
	}
	timer += time.Duration(rand.Int64N(2*int64(cfg.Flows.GRPCReconnectTimerRandomization)) - int64(cfg.Flows.GRPCReconnectTimerRandomization))
	if timer < 0 {
		return time.Second
	}
	return timer
