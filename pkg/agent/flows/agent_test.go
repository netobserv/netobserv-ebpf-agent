...
func TestNew_InvalidConfigs(t *testing.T) {
	for _, tc := range []struct {
		d string
		c config.Agent
	}{{
		d: "invalid export type",
		c: config.Agent{Common: config.Common{Export: "foo"}},
	}, {
		d: "GRPC: missing host",
		c: config.Agent{Common: config.Common{Export: "grpc", TargetPort: 3333}},
	}, {
		d: "GRPC: missing port",
		c: config.Agent{Common: config.Common{Export: "grpc", TargetHost: "flp"}},
	}, {
		d: "Kafka: missing brokers",
		c: config.Agent{Common: config.Common{Export: "kafka"}},
	}} {
...
func TestNewi_ParseFlowFilterRules(t *testing.T) {
	tests := []struct {
		name            string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			filterRules, err := common.ParseFlowFilterRules(tt.flowFilterRules)
