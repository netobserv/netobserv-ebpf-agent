...
func FlowToPB(fr *flow.Record, s *ovnobserv.SampleDecoder) *Record {
	...
	if s != nil {
		for _, ovsMetadata := range fr.Metrics.OvsDpKeys {
			if !flow.AllZerosMetaData(ovsMetadata) {
				if md, err := s.DecodeCookie8Bytes(ovsMetadata); err == nil {
					protoLog.Debugf("OVSMetadata %v decoded OVSCookie: %v", ovsMetadata, md)
					pbflowRecord.OvsMetadata = append(pbflowRecord.OvsMetadata, md)
				} else {
					protoLog.Errorf("unable to decode OVS cookie: %v", err)
				}
			}
		}
	}
...
func PBToFlow(pb *Record) *flow.Record {
	...
	if len(pb.GetOvsMetadata()) != 0 {
		...
		protoLog.Debugf("decoded OVS monitor metadata: %v", out.OvsMonitorMD)
	}
	return &out
}
