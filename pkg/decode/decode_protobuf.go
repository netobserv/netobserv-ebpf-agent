...
func RecordToMap(fr *model.Record) config.GenericMap {
	...
	if fr.Metrics.AdditionalMetrics != nil {
		...
		out["TlsVerdict"] = tlsVerdictToStr(fr.Metrics.AdditionalMetrics.Verdict)
		if fr.Metrics.AdditionalMetrics.TlsMsg.Size != 0 {
			tm := fr.Metrics.AdditionalMetrics.TlsMsg
			out["TlsMsgSize"] = tm.Size
			out["TlsMsgLocalPort"] = tm.LocalPort
			out["TlsMsgRemotePort"] = tm.RemotePort
			out["TlsMsgFamily"] = tlsFamilyToStr(tm.Family)
			out["TlsMsgTlsContentType"] = tlsContentTypeToStr(tm.TlsContentType)
			if tm.TlsHandshakeType != 0 {
				out["TlsMsgTlsHandshakeType"] = tlsHandshakeTypeToStr(tm.TlsHandshakeType)
			}
			if tm.TlsAlertLevel != 0 {
				out["TlsMsgTlsAlertLevel"] = tlsAlertLevelToStr(tm.TlsAlertLevel)
			}
			if tm.TlsAlertDesc != 0 {
				out["TlsMsgTlsAlertDesc"] = tlsAlertDescToStr(tm.TlsAlertDesc)
			}
		}
	}
...
func tlsVerdictToStr(v uint8) string {
	switch v {
	case 1: // SK_PASS
		return "pass"
	case 0: // SK_DROP
...
func tlsFamilyToStr(family uint8) string {
	switch family {
	case 2: // AF_INET
		return "IPv4"
	case 10: // AF_INET6
...
func tlsContentTypeToStr(contentType uint8) string {
	switch contentType {
	case 20:
...
func tlsHandshakeTypeToStr(handshakeType uint8) string {
...
func tlsAlertLevelToStr(alertLevel uint8) string {
	switch alertLevel {
	case 1:
		return "warning"
	case 2:
...
func tlsAlertDescToStr(alertDesc uint8) string {
	switch alertDesc {
	case 0:
		return "close_notify"
	case 10:
