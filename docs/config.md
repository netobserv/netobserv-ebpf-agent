...
* `ENABLE_GOTLS_TRACKING` (default: `false`). Enables Go `crypto/tls` plaintext capture. **Recommended:** set `peer_ip` / `peer_cidr` in `FLOW_FILTER_RULES` (or `TLS_PLAINTEXT_PID_ALLOWLIST`) to scope uprobe discovery; without peer scope the agent hooks all non-excluded Go binaries on the node and logs a warning.
* `GOTLS_ELF_PATH` (default: unset). Optional path to pin a single Go binary instead of auto-discovery.
* `GOTLS_WRITE_OFFSET` / `GOTLS_READ_OFFSET` (default: `0`). Optional manual uprobe offsets overriding auto-discovery.
* `GOTLS_CAPTURE_READ` (default: `false`). **Experimental — can crash Go TLS servers.** When `true`, hooks `crypto/tls.(*Conn).Read` (entry uprobe + uretprobe). The CLI does not enable this; use write-only capture via `--enable_gotls` unless you explicitly opt in on the agent.
* `GOTLS_READ_RET_SITES` (default: `false`). When `true` with `GOTLS_CAPTURE_READ`, attaches at every `RET` in `Read` (legacy eCapture-style). Prefer the default uretprobe; per-RET hooks can destabilize hooked Go binaries.
* `ENABLE_KTLS_TRACKING` (default: `false`). Enables kernel TLS plaintext capture via cgroup sockops and sk_msg hooks.
...
* **5-tuple enrichment** uses socket fd when available: OpenSSL `SSL_set_fd` uprobe + `SSL*`→BIO→fd fallback; GoTLS walks `*tls.Conn`→`net.conn`→`Sysfd`; kTLS embeds the kernel 5-tuple from `sk_msg`. Fd is resolved to `/proc/net/tcp` inode for the exact connection.
