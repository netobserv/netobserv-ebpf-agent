...
_Please also refer to [pkg/config/agent.go](../pkg/config/agent.go) and the `pkg/config/flows` / `pkg/config/packets` subpackages, which are the primary source of truth._
...
* `DNS_TRACKING_PORT` (default: `53`). Comma-separated list of ports to track for DNS traffic (e.g., `53,5353,8053`). Maximum of 8 ports supported. Only used when `ENABLE_DNS_TRACKING` is enabled.
...
* `TLS_PLAINTEXT_PROCESS_ALLOWLIST` (default: unset). Comma-separated list of process names (matched against `/proc/<pid>/comm`) to scope uprobe attachment. When set, only processes whose comm matches an entry in this list are hooked. Infrastructure processes (kubelet, crio, etc.) are always excluded regardless of this setting. **Recommended for production** — without an allowlist, uprobes attach to all non-infrastructure processes.
