...
static __always_inline int do_flow_filter_lookup(flow_id *id, struct filter_key_t *key,
                                                 ...
                                                 u16 flags, u32 drop_reason, u32 *sampling, u8 direction) {
    int result = 0;
...
static __always_inline int is_flow_filtered(flow_id *id, filter_action *action, u16 flags,
                                            u32 drop_reason, u16 eth_protocol, u32 *sampling, u8 direction) {
    ...
    *action = MAX_FILTER_ACTIONS;
    ...
    result = do_flow_filter_lookup(id, &key, action, len, offset, flags, drop_reason, sampling, direction);
    ...
    return do_flow_filter_lookup(id, &key, action, len, offset, flags, drop_reason, sampling, direction);
}
