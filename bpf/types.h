...
// Per-CPU sequence width. ID = ((cpu + 1) << ENDPOINT_ID_SEQ_BITS) | seq.
#define ENDPOINT_ID_SEQ_BITS 20
#define ENDPOINT_ID_SEQ_MASK ((u32)((1u << ENDPOINT_ID_SEQ_BITS) - 1))
...
// Per-CPU next sequence. Packed as ((cpu+1) << 20) | seq so IDs stay unique
// without bpf_spin_lock (forbidden in tracing programs) or BPF fetch-add
// (XADD does not return the previous value on Kernel 5.8+).
typedef struct endpoint_id_state_t {
    u32 next;
} endpoint_id_state;
...
const static struct endpoint_id_state_t *unused19 __attribute__((unused));
