...
static __always_inline void bpf_sock_ops_ip(struct bpf_sock_ops *skops) {
    ...
    case AF_INET:
        ...
        __builtin_memcpy(skk.remote_ip + sizeof(ip4in6), &skops->remote_ip4, sizeof(skops->remote_ip4));
        __builtin_memcpy(skk.local_ip + sizeof(ip4in6), &skops->local_ip4, sizeof(skops->local_ip4));
        break;
    case AF_INET6:
        return;
    }
...
static __always_inline int find_update_flow(struct sk_msg_md *msg, int verdict) {
    ...
    id.if_index = msg->sk->bound_dev_if;
